...
and publishes to GHCR. Tags carry **no `v` prefix** — tag `0.0.1`, not `v0.0.1`. Two
image tags are pushed per build, both pointing at the same digest:
...
The immutable form is `<target>-<lsNNN>-<version>`, where `lsNNN` is the LinuxServer
...
Only `coder` is released today; add a stage by appending it to `RELEASE_TARGETS` in the
workflow's `prepare` job.
